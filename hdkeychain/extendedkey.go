...
func HDPrivateKeyToPublicKeyID(id []byte) ([]byte, error) {
	if len(id) != 4 {
		return nil, fmt.Errorf("ErrUnknownHDKeyID")
	}
...
func (k *ExtendedKey) Address(pkhaid [2]byte) (*address.AddressPubKeyHash, error) {
	...
	return address.NewAddressPubKeyHash(pkHash, pkhaid)
}
...
func (k *ExtendedKey) IsForNet(hdId [4]byte) bool {
	return bytes.Equal(k.version, hdId[:])
}
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	expectedCheckSum := chainhash.HashFuncB(payload)[:4]
